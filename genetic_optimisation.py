...
class genetic_optimisation:
    def __init__(model, # model needs to be a class, with methods build(self.x_train,self.y_train, hp1), train(epochs), evaluate(self.x_test, self.x_test)
        ...
        if keep is not None: # keep 20% each generation, and generate 80% random samples unless otherwise specified
            self.keep = floor(keep*self.size)
        else:
            self.keep = floor(0.2*self.size)
    ...
    def train(self):
        if self.param_one is not None:
            ...
            for gen in range(self.generations):
                ...
                keep_metrics = performance.tree[-self.keep:] # array of the highest performing metrics
